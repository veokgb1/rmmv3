...
### SX.5 垫资报销与公私隔离审批流（长期规划）

> **业务背景**：员工垫资出差、用私人账户支付公司费用后，
> 需要一条完整的从"垫资录入" → "报销审批" → "款项到账" → "轧账核销"的闭环流程。

#### 底层预留（已完成 ✅）
- `Transaction.status: 'expected' | 'cleared' | 'void'`
  — 垫资账单以 `expected` 状态录入，报销到账后更新为 `cleared`，取消报销则 `void`
- `Transaction.offsetByTxId?: string`
  — 报销到账的那条收入记录携带此字段，指向原垫资支出记录，UI 可聚合显示净额

#### SX 阶段实现（待开发）
- [ ] **公私账套隔离模式**：在账套级别标记是否启用"垫资报销"工作流
- [ ] **报销申请单**：员工在私人账套中发起申请，数据推送至企业账套的"待审批"队列
- [ ] **审批流 UI**：企业账套管理员在 `LedgerSettingsPage` 中审批/驳回报销申请
- [ ] **自动轧账**：报销到账后，系统自动将原 `expected` 记录状态更新为 `cleared`，
  并创建对应的 `offsetByTxId` 关联记录
- [ ] **公私拆账视图**：按 `offsetByTxId` 聚合，展示"垫资-报销"配对账单及差额

### SX.6 跨账套数据同步与血缘追溯（Data Pedigree）
